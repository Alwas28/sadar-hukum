<x-admin-layout title="Tambah Pengumuman">
    ...
    <div class="flex items-center gap-3">
      ...
      <div>
        <div class="flex items-center gap-2">
          ...
          <h1 class="font-display text-2xl font-bold text-gray-800">Tambah Pengumuman</h1>
        </div>
        <p class="text-sm text-gray-500 mt-0.5 ml-9">Buat pengumuman resmi untuk portal publik desa</p>
      </div>
    </div>

    <form method="POST" action="{{ route('admin.pengumuman.store') }}" enctype="multipart/form-data" id="pengumuman-form"
          x-data="{ preview: null, handleFile(e) { const f = e.target.files[0]; if (f) this.preview = URL.createObjectURL(f); } }">
      @csrf
      ...
      <div class="grid gap-5">
        ...
        <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 grid sm:grid-cols-2 gap-4">
          <div>
            ...
            <div class="grid grid-cols-2 gap-2">
              @foreach(['penting' => ['Penting','red'], 'info' => ['Info','amber'], 'kegiatan' => ['Kegiatan','blue'], 'umum' => ['Umum','gray']] as $val => [$label, $color])
        ...
        <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
          ...
          <input type="text" name="judul" value="{{ old('judul') }}" placeholder="Tulis judul pengumuman..."
                 class="w-full px-4 py-2.5 text-sm border rounded-xl focus:outline-none focus:border-hijau-400 focus:ring-2 focus:ring-hijau-100 transition-all
        ...
        <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
          <label class="block text-sm font-semibold text-gray-700 mb-3">
            Foto Pendukung <span class="font-normal text-gray-400">(opsional · JPG, PNG, WebP · maks 2MB)</span>
          </label>
          <div class="relative">
        ...
        <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <label class="flex items-start gap-3 cursor-pointer">
            ...
            <div>
              <p class="text-sm font-semibold text-gray-800">Publikasikan langsung</p>
