<x-admin-layout title="Tulis Berita">
    ...
    <div class="flex items-center gap-3">
      ...
      <div>
        <div class="flex items-center gap-2">
          ...
          <h1 class="font-display text-2xl font-bold text-gray-800">Tulis Berita</h1>
        </div>
        <p class="text-sm text-gray-500 mt-0.5 ml-9">Buat artikel berita untuk portal publik desa</p>
      </div>
    </div>

    <form method="POST" action="{{ route('admin.berita.store') }}" enctype="multipart/form-data" id="berita-form"
          x-data="{
            preview: null,
      ...
      <div class="grid gap-5">
        ...
        <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 grid sm:grid-cols-2 gap-4">
            ...
            <div class="grid grid-cols-2 gap-2">
              ...
              <label class="relative cursor-pointer">
                <input type="radio" name="kategori" value="{{ $val }}"
                       {{ old('kategori', 'info') === $val ? 'checked' : '' }} class="peer sr-only"/>
          ...
          <div>
            ...
            <input type="date" name="tanggal_publikasi" value="{{ old('tanggal_publikasi', date('Y-m-d')) }}"
        ...
        <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 space-y-4">
          ...
          <div>
            <label class="block text-sm font-semibold text-gray-700 mb-1.5">
              Slug <span class="font-normal text-gray-400">(URL berita · otomatis dari judul)</span>
            </label>
          ...
          <div>
            ...
            <textarea name="ringkasan" rows="2" placeholder="Paragraf pembuka yang merangkum isi berita..."
                      class="w-full px-4 py-2.5 text-sm border border-gray-200 rounded-xl focus:outline-none focus:border-langit-400 focus:ring-2 focus:ring-langit-100 transition-all resize-none">{{ old('ringkasan') }}</textarea>
          </div>
        </div>
        ...
        <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
          <label class="block text-sm font-semibold text-gray-700 mb-3">
            Foto Utama <span class="font-normal text-gray-400">(sangat dianjurkan · JPG, PNG, WebP · maks 2MB)</span>
          </label>
          <div class="relative">
            ...
            <div class="border-2 border-dashed rounded-xl transition-all"
                 :class="preview ? 'border-langit-300 bg-langit-50 p-2' : 'border-gray-200 bg-gray-50 p-8 text-center hover:border-langit-300 hover:bg-langit-50'">
              ...
              <template x-if="!preview">
                <div>
                  ...
                  <p class="text-xs text-gray-400 mt-1">Foto yang menarik meningkatkan keterbacaan berita</p>
                </div>
              </template>
            </div>
          </div>
...
<script>
  ...
  @if(old('isi'))
    quill.root.innerHTML = {!! json_encode(old('isi')) !!};
  @endif
